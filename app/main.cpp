...
int main()
{
    ...
    Eigen::Vector3d fluid_begin = Eigen::Vector3d(0.05, 0.05, 0.05);
    std::cout << "fluid_begin: " << fluid_begin.transpose() << std::endl;
    Eigen::Vector3d fluid_end = Eigen::Vector3d(0.1, 0.25, 0.5);
    ...
    // const double boundary_volume = 0.001 * boundary_end.x() * boundary_end.y() *
    // boundary_end.z();
    //  const double boundary_volume = particle_diameter * (2 * boundary_end.x() *
    //  boundary_end.y() +
    //  2
    //  * boundary_end.x() * boundary_end.z() + 2 * boundary_end.y() *
    //  boundary_end.z());
    ...
    learnSPH::geometry::load_n_sample_boundary(boundary_particles_positions, boundary_file,
    ...
    std::vector<Eigen::Vector3d> particles_velocities(particles_positions.size(),
    ...
    const double boundary_particle_density = fluid_rest_density / boundary_particles_positions.size();
    ...
    learnSPH::densities::compute_boundary_densities(
        boundary_particles_densities, boundary_particles_positions, point_set_id_boundary,
        ps_boundary, boundary_particles_mass, cubic_kernel);
    ...
    learnSPH::densities::compute_boundary_masses(
        ...
        fluid_rest_density, cubic_kernel);
    ...
    while (t_simulation < 5) {
        ...
        if (t_simulation >= t_next_frame) {
            ...
            learnSPH::write_particles_to_vtk(filename, particles_positions, particles_pressure,
