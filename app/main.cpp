...
int main() {
  ...
  learnSPH::sampling::fluid_box(
      particles_positions, Eigen::Vector3d(-1.0, -1.0, -1.0),
      Eigen::Vector3d(1.0, 1.0, 1.0), fluid_sampling_distance);

  int end_of_box1 = particles_positions.size() - 1;

  learnSPH::sampling::fluid_box(particles_positions, {1.2, -1.0, -1.0},
                                {3.2, 1.0, 1.0}, fluid_sampling_distance);
  ...
  double boundary_volume = std::min(
      0.0,
  ...
  learnSPH::acceleration::Acceleration acceleration(
      B, v_f, v_b, h, fluid_density, gravity, cubic_kernel);
  ...
  while (t_simulation < 100) {
    ...
    if (t_simulation >= t_next_frame) {
      const std::string filename = "../res/wcsph/wcsph_" +
                                   std::to_string((int)(t_simulation * 1000)) +
                                   ".vtk";
      learnSPH::write_particles_to_vtk(filename, particles_positions);
