...
//     sim_setup.dam_break();
    ...
    learnSPH::acceleration::Acceleration acceleration(
            sim_setup.B, sim_setup.v_f, sim_setup.v_b, h,
            sim_setup.fluid_rest_density, sim_setup.gravity, cubic_kernel);
    learnSPH::timeIntegration::semiImplicitEuler semImpEuler(sim_setup.particle_radius);

    if (sim_setup.boundary_file.size() > 0){
        learnSPH::timeIntegration::semiImplicitEuler semImpEuler(
            sim_setup.particle_radius, sim_setup.boundary_end, sim_setup.boundary_begin);
    }

    // Load simulation geometry
    std::vector<Eigen::Vector3d> boundary_particles_positions;
    learnSPH::geometry::load_n_sample_boundary(
            boundary_particles_positions, sim_setup.boundary_file, boundary_sampling_distance);
    ...
    learnSPH::geometry::load_n_sample_fluids(particles_positions, particles_velocities, sim_setup.fluid_begin, sim_setup.fluid_end, fluid_sampling_distance, sim_setup.fluid_velocities);
    ...
    learnSPH::densities::compute_boundary_masses(
            ...
            point_set_id_boundary, ps_boundary, sim_setup.fluid_rest_density, cubic_kernel);
    ...
    while (t_simulation < 5) {
        ...
        // Compute dt
        dt_cfl = 0.5 * sim_setup.particle_radius * (1 / std::min(100.0, std::sqrt(semImpEuler.v_max)));
        ...
        if (count_del > 0 && sim_setup.boundary_file.size() > 0){
        ...
        if (t_simulation >= t_next_frame) {
            const std::string filename =
                    "./res/" + sim_setup.assignment + "/" + simulation_timestamp + "/sim_" +
