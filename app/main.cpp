...
int main()
{
	...
	learnSPH::sampling::fluid_box(particles_positions
	...
	unsigned int point_set_id_boundary = nsearch.add_point_set(boundary_particles_positions.front().data()
															 , boundary_particles_positions.size());
	unsigned int point_set_id_fluid = nsearch.add_point_set(particles_positions.front().data()
	...
	learnSPH::densities::compute_boundary_masses(boundary_particles_masses
												, boundary_particles_positions
												, point_set_id_boundary
												, ps_boundary
												, fluid_density
												, cubic_kernel);
	double boundary_volume = std::min(0.0, std::accumulate(boundary_particles_masses.begin()
											, boundary_particles_masses.end()
											, decltype(boundary_particles_masses)::value_type(0.0))/fluid_density);
	...
	std::vector<Eigen::Vector3d> particles_velocities(particles_positions.size(), Eigen::Vector3d(0.0, 0.0, 0.0));
	...
	learnSPH::acceleration::Acceleration acceleration(B
													, v_f
													, v_b
													, h
													, fluid_density
	...
	while (t_simulation < 100) {
        ...
        auto duration_neighborhood = std::chrono::duration_cast<std::chrono::microseconds>(stop_neighborhood - start_neighborhood);
        ...
        auto duration_density = std::chrono::duration_cast<std::chrono::microseconds>(stop_density - start_density);
		...
		acceleration.pressure(particles_pressure
							, particles_densities
							, fluid_density);
		auto stop_pressure = std::chrono::high_resolution_clock::now();
        auto duration_pressure = std::chrono::duration_cast<std::chrono::microseconds>(stop_pressure - start_pressure);
		...
		acceleration.accelerations(particles_accelerations
									, particles_densities
									, particles_pressure
									, point_set_id_fluid
									, point_set_id_boundary
        ...
        auto duration_acceleration = std::chrono::duration_cast<std::chrono::microseconds>(stop_acceleration - start_acceleration);
		...
		semImpEuler.integrationStep(particles_positions
									, particles_velocities
									, particles_accelerations
									, dt);
		auto stop_integrate = std::chrono::high_resolution_clock::now();
        auto duration_integrate = std::chrono::duration_cast<std::chrono::microseconds>(stop_integrate - start_integrate);
