...
std::vector<std::string> learnSPH::neighborhood_search::brute_force_neighbor(
    const double h, std::vector<Eigen::Vector3d> particles, int particle_index,
    double beta) {
...
void learnSPH::neighborhood_search::brute_force_neighbors(
    const double h, std::vector<Eigen::Vector3d> particles, double beta) {
  for (int i = 0; i < (int)particles.size(); i++) {
    brute_force_neighbor(h, particles, i, beta);
  }
}

std::vector<std::vector<int>> learnSPH::neighborhood_search::brute_force_search(
    const double h, std::vector<Eigen::Vector3d> particles, double beta) {
