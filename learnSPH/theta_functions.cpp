...
learnSPH::theta_functions::FluidThetaFunction::FluidThetaFunction(double c, double cell_width, double support_radius, uint n_vy, uint n_vz){
    this->c = c;
        ...
        for (int i=lower_x_abb; i <upper_x_abb + 1; i++){
            for (int j=lower_y_abb; j < upper_y_abb + 1; j++){
                for(int k=lower_z_abb; k < upper_z_abb + 1; k++){
                    ...
                    Eigen::Vector3d vertex_pos = Eigen::Vector3d(i*cell_width,j*cell_width,k*cell_width);
