...
void learnSPH::sampling::fluid_box(std::vector<Eigen::Vector3d> &particles,
...
void learnSPH::sampling::triangle_mesh(std::vector<Eigen::Vector3d> &particles,
...
learnSPH::sampling::_find_edges(const std::vector<std::array<int, 3>> &triangles)
{
        ...
        for (int loc_node_i = 0; loc_node_i < 3; loc_node_i++) {
            for (int loc_node_j = loc_node_i + 1; loc_node_j < 3; loc_node_j++) {
                std::array<int, 2> edge = {
                    std::min(triangles[elem_i][loc_node_i], triangles[elem_i][loc_node_j]),
                    std::max(triangles[elem_i][loc_node_i], triangles[elem_i][loc_node_j])};

                // Check if this edge already exist
                bool found = false;
                for (int glob_node_j :
                     node_edge_map[edge[0]]) { // Loop through the nodes connected to glob_node_i
...
void learnSPH::sampling::_sample_triangle(std::vector<Eigen::Vector3d> &particles,
                                          const Eigen::Vector3d &a, const Eigen::Vector3d &b,
                                          const Eigen::Vector3d &c, const double sampling_distance)
{
    ...
    const double triangle_radius =
        std::max((a - triangle_centroid).norm(),
                 std::max((b - triangle_centroid).norm(), (c - triangle_centroid).norm()));
    if (triangle_radius < sampling_distance) {
        particles.push_back(triangle_centroid);
    }

    // Otherwise use a grid to place the particles and discard the ones outside of the extended
    // triangle
    else {
        ...
        const Eigen::Vector3d l_a =
            a + ((a - b).normalized() + (a - c).normalized()) * 2.0 * particle_radius;
        const Eigen::Vector3d l_b =
            b + ((b - a).normalized() + (b - c).normalized()) * 2.0 * particle_radius;
        const Eigen::Vector3d l_c =
            c + ((c - a).normalized() + (c - b).normalized()) * 2.0 * particle_radius;

        const double enlarged_sampling_length_u =
            std::max(length_u, (c - a).dot(u)) + particle_radius;
        ...
        for (int i = 0; i < nu; i++) {
            ...
            for (int j = 0; j < nv; j++) {
                const Eigen::Vector3d particle =
                    a + u * i * hexa_step_u + v * (j * particle_diameter + offset);
...
bool learnSPH::sampling::_point_on_triangle(const Eigen::Vector3d &a, const Eigen::Vector3d &b,
...
void learnSPH::sampling::fluid_spheres(std::vector<Eigen::Vector3d> &particles,
                                       ...
                                       const std::vector<Eigen::Vector3d> fluid_velocities)
{
    ...
    for (int idx = 0; idx < spheres.size(); ++idx) {
        ...
        y      = 0;
        z      = 0;
        l      = spheres[idx].radius * 2 / (particle_radius * 2);
        corner = -l / 2. * 2 * particle_radius;
        radius_squared =  spheres[idx].radius  * spheres[idx].radius;
            ...
            for (int i = 0; i < 2*l; ++i) {
                for (int j = 0; j < 2*l; ++j) {
                    ...
                    if ((x * x + y * y + z * z) <= radius_squared) {
                        Eigen::Vector3d new_particle = Eigen::Vector3d(x, y, z) + spheres[idx].origin;
        ...
        for (int i = begin_vector; i < particles.size(); ++i) {
            velocities.push_back(fluid_velocities[idx]);
        }
        begin_vector = velocities.size();
    }
}